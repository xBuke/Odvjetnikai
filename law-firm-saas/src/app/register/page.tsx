    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-800 px-4 sm:px-6 lg:px-8">
      ...
      >
        <div className="text-center space-y-6">
          <div className="w-20 h-20 bg-gradient-to-br from-gold to-gold-dark rounded-full flex items-center justify-center mx-auto shadow-lg">
          ...
          <div className="space-y-3">
            ...
            <p className="text-foreground font-medium bg-slate-100 dark:bg-slate-700 rounded-lg py-2 px-4">
              {email}
            </p>
          </div>
            ...
            <div className="flex items-start">
              ...
              <div className="text-left">
                ...
                <p className="text-xs text-blue-700 dark:text-blue-300 mt-1">
                  Nakon potvrde e-maila, automatski ćete dobiti besplatni 7-dnevni trial i biti preusmjereni na dashboard.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-3">
            <p className="text-sm text-muted-foreground">
              Niste primili e-mail? Provjerite spam mapu ili:
            </p>
              ...
              <button
                onClick={async () => {
                  try {
                    ...
                    if (error) {
                      alert(`Greška pri slanju e-maila: ${error.message}`);
                    } else {
                      alert('E-mail za potvrdu je ponovno poslan!');
                    }
                  } catch (error) {
                    alert('Dogodila se greška. Molimo pokušajte ponovno.');
                  }
                }}
                className="flex-1 py-2 px-4 bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 text-foreground font-medium rounded-lg transition-colors"
              >
                Pošalji ponovno
              </button>
              <button
                onClick={() => setSignupSuccess(false)}
                className="flex-1 py-2 px-4 bg-gradient-to-r from-gold to-gold-light hover:from-gold-light hover:to-gold-dark text-navy font-semibold rounded-lg transition-all duration-200"
              >
                Zatvori
              </button>
            </div>
